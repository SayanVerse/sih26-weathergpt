...
export const WeatherMetrics: React.FC<WeatherMetricsProps> = ({ current, isLoading = false }) => {
    ...
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3.5">
        {Array.from({ length: 8 }).map((_, i) => (
          <Skeleton key={i} className="h-28 rounded-2xl bg-zinc-200 dark:bg-zinc-900" />
        ))}
      </div>
    );
  }
  ...
  const metrics = [
    ...
    },
    {
      ...
      icon: Wind,
      iconColor: 'text-teal-600 dark:text-blue-400',
    },
    {
      id: 'metric-uv',
      label: 'UV Index',
    ...
    },
    {
      ...
      value: `${aqi} AQI`,
      badge: 'Good',
      badgeColor: 'text-emerald-700 dark:text-green-400',
      badgeBg: 'bg-emerald-50 dark:bg-green-500/10 border-emerald-200 dark:border-green-500/30',
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3.5">
      {metrics.map((m) => {
        ...
        return (
          <Card
            key={m.id}
            className="p-4 bg-white/80 hover:bg-white dark:bg-zinc-900/80 dark:hover:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 transition-all duration-200 shadow-sm dark:shadow-none"
          >
            <div className="flex items-center justify-between gap-2 mb-2">
              <span className="text-xs font-medium text-zinc-600 dark:text-zinc-400 truncate">
                {m.label}
              </span>
              <div className="p-1.5 rounded-lg bg-zinc-100 dark:bg-zinc-800/80 border border-zinc-200 dark:border-zinc-700/60 shrink-0">
                <Icon className={`w-3.5 h-3.5 ${m.iconColor}`} />
              </div>
            </div>

            <div className="flex items-baseline gap-2 mb-1">
              <span className="text-xl sm:text-2xl font-bold text-zinc-900 dark:text-zinc-100 font-mono-numbers tracking-tight">
                {m.value}
              </span>
              {m.badge && (
                <span
                  className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-md border ${m.badgeBg || 'bg-zinc-100 dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700'} ${m.badgeColor || 'text-zinc-700 dark:text-zinc-300'}`}
                >
                  {m.badge}
                </span>
              )}
