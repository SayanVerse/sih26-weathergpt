...
const ParticleText = forwardRef<ParticleTextRef, ParticleTextProps>(({
  ...
  const internalRef = useRef<{ particles: Particle[], gatherStart: number, gathering: boolean }>({
    particles: [], gatherStart: 0, gathering: false
  });
  ...
  useImperativeHandle(ref, () => ({
    explode: () => {
      ...
      state.particles.forEach(particle => {
        ...
        // Target their exact original scattered positions
        particle.targetX = particle.initialX;
        particle.targetY = particle.initialY;
        particle.delay = (1 - particle.depth) * 150; // slight reverse stagger
      });
  ...
  useEffect(() => {
    ...
    const syncInternalRef = () => {
      internalRef.current.particles = particles;
      internalRef.current.gathering = gathering;
      internalRef.current.gatherStart = gatherStart;
    };
    ...
    const render = (now: number): void => {
      ...
      // Update from internal ref in case explode was called
      gathering = internalRef.current.gathering;
      gatherStart = internalRef.current.gatherStart;
      ...
      if (gathering && complete) {
        gathering = false;
        internalRef.current.gathering = false;
      }
