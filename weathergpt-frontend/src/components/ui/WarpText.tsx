...
const WarpText = ({
  ...
  useEffect(() => {
    ...
    const onPointerLeave = (): void => { pointer.activeTarget = 0; };
    const onContextLost = (event: Event): void => { event.preventDefault(); contextLost = true; if (raf) cancelAnimationFrame(raf); raf = 0; };
    ...
    intersectionObserver = new IntersectionObserver(([entry]: IntersectionObserverEntry[]) => {
  ...
  return (
    <div
      ref={containerRef}
      className={`relative block overflow-hidden isolate ${className}`.trim()}
      style={style}
      role="img"
      aria-label={text}
    />
